...
function engineSupportsSqlMigrations(engine) {
...
export async function listMigrations(userId, databaseId) {
...
export async function applyMigration(userId, databaseId, input) {
    ...
    try {
        ...
        const friendly = explainDbRuntimeError(isMysqlFamily(database.engine) ? 'mysql' : 'postgres', error);
