...
import { platformMongoConfig, platformMongoPublicConfig, platformMysqlConfig, platformMysqlPublicConfig, platformPgConfig, platformPgPublicConfig, } from '../../config/env.js';
import { deleteRow, dropPostgresManagedDatabase, getTableData, insertRow, listSchema, provisionDatabase, restoreBackupPayload as restorePostgresBackupPayload, runSql, testConnection, truncatePublicTable, updateRow, } from '../../adapters/postgres/adapter.js';
import { applyConstructorSchema, exportConstructorSchema } from '../../adapters/postgres/constructor.js';
import { createMongoCollection, deleteMongoCollection, deleteMongoDocument, clearMongoCollection, exportMongoBackupPayload, getMongoCollectionData, getMongoVisual, insertMongoDocument, listMongoCollections, provisionMongoDatabase, restoreMongoBackupPayload, testMongoConnection, dropMongoManagedDatabase, updateMongoDocument, } from '../../adapters/mongodb/adapter.js';
import { applyMysqlConstructorSchema, deleteMysqlRow, exportMysqlBackupPayload, exportMysqlConstructorSchema, getMysqlTableData, insertMysqlRow, listMysqlSchema, provisionMysqlDatabase, restoreMysqlBackupPayload, runMysqlMigrationSql, runMysqlSql, testMysqlConnection, dropMysqlManagedDatabase, normalizeMysqlTcpHost, truncateMysqlTable, updateMysqlRow, } from '../../adapters/mysql/adapter.js';
...
function getDisplayEngine(engine) {
    if (engine === 'mongodb')
        return 'MongoDB';
    if (engine === 'mysql')
        return 'MySQL';
...
async function withReadableDbError(engine, fn) {
    try {
        return await fn();
    }
    catch (error) {
        if (error instanceof HttpError)
            throw error;
        throw new HttpError(400, explainDbRuntimeError(engine, error));
    }
}
/** Блокирует запросы к живой БД, пока карточка в проблемном статусе (создание, миграция, ошибка). */
export function assertDatabaseReadyForWork(database) {
    switch (database.status) {
        ...
        case DatabaseStatus.MIGRATING:
            throw new HttpError(409, 'С базой выполняется перенос или смена СУБД. Дождитесь окончания или откройте вкладку «Обзор».');
        case DatabaseStatus.ERROR:
            throw new HttpError(409, 'У базы статус «ошибка». На вкладке «Обзор» нажмите «Проверить подключение» или удалите карточку.');
        default:
            break;
    }
}
function getConnectionString(input, engine) {
    if (engine === 'mongodb') {
...
function getPublicConnection(database, overrides) {
...
function slugifyDatabaseName(input, prefix) {
...
function parseOptionalInitialSql(raw) {
...
export async function requireDatabaseForUser(databaseId, userId) {
...
function assertSafeMongoCollectionName(tableParam) {
...
export function getDecryptedConnection(database) {
    const common = {
        host: database.host,
        port: database.port,
        database: database.databaseName,
...
async function getLiveConnectionsCount(database) {
    const engine = normalizeEngine(database.engine);
    if (engine === 'mongodb')
        return database.connections ?? 0;
    if (engine === 'mysql') {
        try {
            const result = await runMysqlSql(getDecryptedConnection(database), 'SELECT COUNT(*) AS connections FROM information_schema.processlist WHERE db = DATABASE()', []);
    ...
    try {
        const result = await runSql(getDecryptedConnection(database), `
...
export async function toPublicDatabase(database) {
...
export async function getDatabaseConnectionInfo(userId, databaseId, overrides) {
...
function readConstructorState(raw) {
    if (!raw)
        return {};
    try {
...
function mergeConstructorState(schema, state) {
...
function buildConstructorState(schema) {
    let colors = {};
    if (schema.tableColors !== undefined) {
        ...
        colors = Object.fromEntries((schema.tables ?? []).filter((table) => table.color && String(table.color).trim()).map((table) => [table.name, String(table.color)]));
    }
    return {
...
function computeConstructorStateJsonAfterSqlRestore(schema, previousConstructorStateJson) {
...
export async function listDatabases(userId) {
    ...
    return Promise.all(databases.map(async (database) => ({
        ...(await toPublicDatabase(database)),
...
export async function moveDatabaseToProject(userId, databaseId, targetProjectId) {
...
export async function syncManagedDatabaseHealth(userId, databaseId) {
    ...
    try {
        if (engine === 'mongodb') {
            await testMongoConnection(getDecryptedConnection(database));
        }
        else if (engine === 'mysql') {
            await testMysqlConnection(getDecryptedConnection(database));
        }
        else {
            await testConnection(getDecryptedConnection(database));
        }
    }
    catch (error) {
        throw new HttpError(400, explainDbRuntimeError(engine, error));
    }
    const data = { status: DatabaseStatus.RUNNING };
    if (engine === 'mysql') {
...
export async function registerExistingPostgres(userId, input) {
    ...
    return { ...(await toPublicDatabase(database)), myProjectRole: member.role.toLowerCase() };
}
export async function registerExistingMongo(userId, input) {
    const member = await requireProjectRole(input.projectId, userId, UserRole.ADMIN, 'Подключать внешние базы могут только администраторы проекта');
    const project = await prisma.project.findUnique({ where: { id: input.projectId } });
    if (!project)
        throw new HttpError(404, 'Проект не найден.');
...
export async function registerExistingMysql(userId, input) {
    const member = await requireProjectRole(input.projectId, userId, UserRole.ADMIN, 'Подключать внешние базы могут только администраторы проекта');
    const project = await prisma.project.findUnique({ where: { id: input.projectId } });
    if (!project)
        throw new HttpError(404, 'Проект не найден.');
...
export async function createManagedPostgresDatabase(userId, input) {
...
export async function createManagedMongoDatabase(userId, input) {
    const member = await requireProjectRole(input.projectId, userId, UserRole.ADMIN, 'Создавать управляемые базы могут только администраторы проекта');
    const project = await prisma.project.findUnique({ where: { id: input.projectId } });
    if (!project)
        throw new HttpError(404, 'Проект не найден.');
...
export async function createManagedMysqlDatabase(userId, input) {
    const member = await requireProjectRole(input.projectId, userId, UserRole.ADMIN, 'Создавать управляемые базы могут только администраторы проекта');
    const project = await prisma.project.findUnique({ where: { id: input.projectId } });
    if (!project)
        throw new HttpError(404, 'Проект не найден.');
...
export async function migrateManagedSqlEngine(userId, databaseId, targetEngine) {
    ...
    const snapshot = await buildFullBackupPayload(database);
    const converted = convertSqlBackupPayloadForEngine(snapshot, targetNorm === 'mysql' ? 'mysql' : 'postgresql');
    await prisma.managedDatabase.update({
        where: { id: database.id },
...
export async function deleteDatabase(userId, databaseId) {
    ...
    await prisma.auditLog.create({
        data: {
            ...
            userId,
            userName: await getUserName(userId),
            details: database.region === 'managed'
...
export async function getSchema(userId, databaseId) {
    const database = await requireDatabaseForUser(databaseId, userId);
    assertDatabaseReadyForWork(database);
    const engine = normalizeEngine(database.engine);
    return withReadableDbError(engine, async () => {
        if (engine === 'mongodb') {
            return listMongoCollections(getDecryptedConnection(database));
        }
        if (engine === 'mysql') {
            return listMysqlSchema(getDecryptedConnection(database));
        }
        return listSchema(getDecryptedConnection(database));
    });
}
export async function getTableRows(userId, databaseId, table, limit, offset) {
    const database = await requireDatabaseForUser(databaseId, userId);
    assertDatabaseReadyForWork(database);
    const engine = normalizeEngine(database.engine);
    return withReadableDbError(engine, async () => {
        if (engine === 'mongodb') {
            return getMongoCollectionData(getDecryptedConnection(database), table, limit, offset);
        }
        if (engine === 'mysql') {
            return getMysqlTableData(getDecryptedConnection(database), table, limit, offset);
        }
        return getTableData(getDecryptedConnection(database), table, limit, offset);
    });
}
export async function executeSql(userId, databaseId, sql, values = []) {
    ...
    return withReadableDbError(engine, async () => {
        if (engine === 'mysql') {
            return runMysqlSql(getDecryptedConnection(database), sql, values);
        }
        return runSql(getDecryptedConnection(database), sql, values);
    });
}
export async function createRow(userId, databaseId, table, data) {
    const { database } = await requireDatabaseProjectRole(databaseId, userId, UserRole.EDITOR, 'Наблюдатель не может менять данные');
    assertDatabaseReadyForWork(database);
    const engine = normalizeEngine(database.engine);
    return withReadableDbError(engine, async () => {
        if (engine === 'mongodb') {
            return insertMongoDocument(getDecryptedConnection(database), table, data);
        }
        if (engine === 'mysql') {
            return insertMysqlRow(getDecryptedConnection(database), table, data);
        }
        return insertRow(getDecryptedConnection(database), table, data);
    });
}
export async function patchRow(userId, databaseId, table, pkColumn, pkValue, patch) {
    const { database } = await requireDatabaseProjectRole(databaseId, userId, UserRole.EDITOR, 'Наблюдатель не может менять данные');
    assertDatabaseReadyForWork(database);
    const engine = normalizeEngine(database.engine);
    return withReadableDbError(engine, async () => {
        if (engine === 'mongodb') {
            return updateMongoDocument(getDecryptedConnection(database), table, pkValue, patch);
        }
        if (engine === 'mysql') {
            return updateMysqlRow(getDecryptedConnection(database), table, pkColumn, pkValue, patch);
        }
        return updateRow(getDecryptedConnection(database), table, pkColumn, pkValue, patch);
    });
}
export async function removeRow(userId, databaseId, table, pkColumn, pkValue) {
    const { database } = await requireDatabaseProjectRole(databaseId, userId, UserRole.EDITOR, 'Наблюдатель не может менять данные');
    assertDatabaseReadyForWork(database);
    const engine = normalizeEngine(database.engine);
    return withReadableDbError(engine, async () => {
        if (engine === 'mongodb') {
            return deleteMongoDocument(getDecryptedConnection(database), table, pkValue);
        }
        if (engine === 'mysql') {
            return deleteMysqlRow(getDecryptedConnection(database), table, pkColumn, pkValue);
        }
        return deleteRow(getDecryptedConnection(database), table, pkColumn, pkValue);
    });
}
export async function truncateTable(userId, databaseId, tableParam) {
    ...
    return withReadableDbError(engine, async () => {
        if (engine === 'mongodb') {
            ...
            return clearMongoCollection(getDecryptedConnection(database), collection);
        }
        assertSafeTableName(tableParam);
        if (engine === 'mysql') {
            await truncateMysqlTable(getDecryptedConnection(database), tableParam);
            return { truncated: true };
        }
        await truncatePublicTable(getDecryptedConnection(database), tableParam);
        return { truncated: true };
    });
}
export async function getConstructor(userId, databaseId) {
    const database = await requireDatabaseForUser(databaseId, userId);
    assertDatabaseReadyForWork(database);
    const engine = normalizeEngine(database.engine);
    if (engine === 'mongodb') {
    ...
    return withReadableDbError(engine, async () => {
        const schema = engine === 'mysql'
            ? await exportMysqlConstructorSchema(conn)
            : await exportConstructorSchema(conn);
        const current = readConstructorState(database.constructorStateJson);
        ...
        if (JSON.stringify(current) !== JSON.stringify(nextState)) {
            await prisma.managedDatabase.update({
                where: { id: database.id },
                data: { constructorStateJson: JSON.stringify(nextState) },
            });
        }
        return mergeConstructorState(schema, nextState);
    });
}
export async function saveConstructorLayout(userId, databaseId, input) {
    ...
    return withReadableDbError(engine, async () => {
        ...
        await prisma.managedDatabase.update({
            where: { id: database.id },
            data: { constructorStateJson: JSON.stringify(next) },
        });
        return { saved: true };
    });
}
export async function applyConstructor(userId, databaseId, schema) {
    ...
    return withReadableDbError(engine, async () => {
        if (engine === 'mysql') {
            await applyMysqlConstructorSchema(conn, schema);
            const freshSchema = await exportMysqlConstructorSchema(conn);
        ...
        await applyConstructorSchema(conn, schema);
        const freshSchema = await exportConstructorSchema(conn);
        const nextState = pruneConstructorState(buildConstructorState(schema), freshSchema.tables.map((table) => table.name));
        await prisma.managedDatabase.update({
            where: { id: database.id },
            data: { constructorStateJson: JSON.stringify(nextState) },
...
export async function getMongoVisualState(userId, databaseId) {
    const database = await requireDatabaseForUser(databaseId, userId);
    assertDatabaseReadyForWork(database);
    ...
    return withReadableDbError('mongodb', async () => getMongoVisual(getDecryptedConnection(database)));
}
export async function createMongoCollectionForUser(userId, databaseId, name) {
    ...
    return withReadableDbError('mongodb', async () => createMongoCollection(getDecryptedConnection(database), name));
}
export async function deleteMongoCollectionForUser(userId, databaseId, name) {
    ...
    return withReadableDbError('mongodb', async () => deleteMongoCollection(getDecryptedConnection(database), name));
}
export async function exportBackupPayloadForDatabase(database) {
    if (normalizeEngine(database.engine) === 'mongodb') {
        return exportMongoBackupPayload(getDecryptedConnection(database));
    }
    if (normalizeEngine(database.engine) === 'mysql') {
        return exportMysqlBackupPayload(getDecryptedConnection(database));
    }
    const { exportBackupPayload } = await import('../../adapters/postgres/adapter.js');
    return exportBackupPayload(getDecryptedConnection(database));
}
export async function restoreBackupPayloadForDatabase(database, payload) {
    if (normalizeEngine(database.engine) === 'mongodb') {
        return restoreMongoBackupPayload(getDecryptedConnection(database), payload);
    }
    if (normalizeEngine(database.engine) === 'mysql') {
        return restoreMysqlBackupPayload(getDecryptedConnection(database), payload);
    }
    const { restoreBackupPayload } = await import('../../adapters/postgres/adapter.js');
    return restoreBackupPayload(getDecryptedConnection(database), payload);
}
/** Полный снимок для бэкапа: данные как раньше + мета + логическая схема (таблицы, связи, позиции ER из платформы). */
export async function buildFullBackupPayload(database) {
