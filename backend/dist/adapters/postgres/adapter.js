...
export function createPostgresPool(connection) {
    return new Pool({
...
export async function dropPostgresManagedDatabase(admin, databaseName, ownerRole) {
...
export async function listSchema(connection) {
    const pool = createPostgresPool(connection);
    try {
...
export async function getTableData(connection, table, limit, offset) {
    const pool = createPostgresPool(connection);
    try {
        const columnsRes = await pool.query(`SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = $1 ORDER BY ordinal_position`, [table]);
...
export async function runSql(connection, sql, values = []) {
    const pool = createPostgresPool(connection);
    try {
...
export async function insertRow(connection, table, data) {
    const pool = createPostgresPool(connection);
    try {
...
export async function updateRow(connection, table, pkColumn, pkValue, patch) {
    const pool = createPostgresPool(connection);
    try {
...
export async function deleteRow(connection, table, pkColumn, pkValue) {
    const pool = createPostgresPool(connection);
    try {
...
export async function truncatePublicTable(connection, table) {
    const pool = createPostgresPool(connection);
    try {
...
export async function exportBackupPayload(connection) {
    const pool = createPostgresPool(connection);
    try {
        const tables = await listSchema(connection);
        const payload = { tables: [] };
        for (const table of tables) {
            const columnsRes = await pool.query(`SELECT
         ...
         ORDER BY a.attnum`, [table.name]);
            const pkRes = await pool.query(`SELECT kcu.column_name
         ...
         ORDER BY kcu.ordinal_position`, [table.name]);
            const primaryKeyColumns = pkRes.rows.map((r) => String(r.column_name));
...
function inferPrimaryKeyFromLogicalSchema(tableName, logicalSchema) {
...
function pgDataTypeFallback(dataType) {
...
export function buildPostgresCreateTableFromBackupColumns(tableName, columns, primaryKeyColumns, logicalSchema) {
    if (!columns?.length) {
    ...
    if (!pk.length)
        pk = inferPrimaryKeyFromLogicalSchema(tableName, logicalSchema);
...
async function pgTableExists(client, tableName) {
    const r = await client.query(`SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = $1) AS e`, [tableName]);
    return Boolean(r.rows[0]?.e);
}
async function getPgFkParentChildEdges(client, backupNames) {
  ...
  `);
    ...
    for (const row of r.rows) {
        ...
        if (backupNames.has(p) && backupNames.has(ch))
            edges.push([p, ch]);
    }
    return edges;
}
...
export async function restoreBackupPayload(connection, payload) {
    const pool = createPostgresPool(connection);
    ...
    try {
        ...
        for (const row of dbTablesRes.rows) {
            ...
            if (backupNames.has(name) || NEVER_DROP_PUBLIC_TABLES.has(name))
                continue;
        ...
        for (const table of backupTables) {
            if (!(await pgTableExists(client, String(table.name)))) {
                const ddl = buildPostgresCreateTableFromBackupColumns(String(table.name), table.columns ?? [], table.primaryKeyColumns, payload.logicalSchema);
        ...
        for (const name of insertOrder) {
            ...
            for (const row of table.rows) {
                ...
                await client.query(`INSERT INTO ${quoteIdent('public')}.${quoteIdent(name)} (${cols}) VALUES (${placeholders})`, values);
            }
        }
