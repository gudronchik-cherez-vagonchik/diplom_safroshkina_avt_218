...
function roleInviteLabel(role: UserRole): string {
  switch (role) {
    case UserRole.OWNER:
      return 'владелец';
    case UserRole.ADMIN:
...
function mapListedProject(project: { id: string; name: string; description: string; createdAt: Date; updatedAt: Date; databases: unknown[]; members: unknown[] }) {
  return {
    id: project.id,
    name: project.name,
    ...
    environments: [{ name: 'production', status: 'running' as const }],
  };
}
...
export async function createProject(userId: string, input: { name: string; description?: string }) {
  ...
  await notifyUser(
    ...
    'Можно добавить базы данных и пригласить участников.',
...
export async function getProject(userId: string, projectId: string) {
  ...
  if (!membership) throw new HttpError(404, 'Проект не найден или нет доступа');
...
export async function listProjectMembers(requesterId: string, projectId: string) {
  const membership = await prisma.projectMember.findUnique({
    where: { projectId_userId: { projectId, userId: requesterId } },
  });
  if (!membership) throw new HttpError(404, 'Проект не найден');
...
export async function previewProjectInvite(token: string) {
...
export async function inviteProjectMemberByUsername(
  actorUserId: string,
  projectId: string,
  usernameRaw: string,
  role: UserRole,
  ...
  if (!target) throw new HttpError(404, 'Пользователь с таким username не найден');
  ...
  await notifyUser(
    ...
    `Приглашение в проект «${project?.name ?? 'Проект'}»`,
    `${actor?.name ?? 'Участник'} пригласил вас как ${roleInviteLabel(role)}. Примите приглашение здесь или по ссылке от коллеги.`,
    {
      inviteToken: token,
      projectId,
