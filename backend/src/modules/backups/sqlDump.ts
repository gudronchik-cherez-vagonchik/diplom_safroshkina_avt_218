...
function pgQI(value: string) {
  return `"${value.replace(/"/g, '""')}"`;
}

function myQI(value: string) {
  return `\`${value.replace(/`/g, '``')}\``;
}

function fkEdgesFromLogicalSchema(logicalSchema: unknown, backupNames: Set<string>): Array<[string, string]> {
  if (!logicalSchema || typeof logicalSchema !== 'object') return [];
  const ls = logicalSchema as {
    kind?: string;
    relations?: Array<{ fromTable: string; toTable: string }>;
  };
  if (ls.kind !== 'sql' || !Array.isArray(ls.relations)) return [];
  const edges: Array<[string, string]> = [];
  for (const r of ls.relations) {
    const parent = String(r.toTable);
    const child = String(r.fromTable);
    if (backupNames.has(parent) && backupNames.has(child)) edges.push([parent, child]);
  }
  return edges;
}
...
function buildPostgresInsert(tableName: string, row: Record<string, unknown>): string {
...
function mysqlEscapeLiteral(val: unknown): string {
  if (val === null || val === undefined) return 'NULL';
...
function buildMysqlCreateTableFromBackupPayload(table: any, logicalSchema: unknown): string {
  const columns = table.columns ?? [];
  if (!columns.length) throw new Error(`Таблица «${table.name}» без колонок в бэкапе`);
  const colNames = new Set(columns.map((c: any) => String(c.COLUMN_NAME)));
  ...
  pk = pk.filter((n: string) => colNames.has(n));
  const parts: string[] = [];
  ...
  if (pk.length) parts.push(`PRIMARY KEY (${pk.map((c: string) => myQI(c)).join(', ')})`);
  return `CREATE TABLE ${myQI(String(table.name))} (\n  ${parts.join(',\n  ')}\n)`;
}

function buildMysqlInsert(tableName: string, row: Record<string, unknown>): string {
  const keys = Object.keys(row);
  if (!keys.length) return '';
...
export function buildPostgresSqlDump(payload: any, includeData: boolean): string {
  const lines: string[] = [
  ...
  for (const name of insertOrder) {
    const t = backupTables.find((x: any) => String(x.name) === name);
    if (!t?.columns?.length) continue;
    lines.push(
      `${buildPostgresCreateTableFromBackupColumns(name, t.columns, t.primaryKeyColumns, payload.logicalSchema)};`,
    );
  }
  if (includeData) {
    for (const name of insertOrder) {
...
export function buildMysqlSqlDump(payload: any, includeData: boolean): string {
  const lines: string[] = ['-- МояБД: MySQL dump из JSON-бэкапа', 'SET FOREIGN_KEY_CHECKS = 0;', 'START TRANSACTION;'];
  const backupTables = payload.tables ?? [];
  const backupNames = new Set(backupTables.map((t: any) => String(t.name)));
  const edges = fkEdgesFromLogicalSchema(payload.logicalSchema, backupNames);
  const insertOrder = topoSortParentsFirst([...backupNames], edges);
