...
import {
  assertDatabaseReadyForWork,
  buildFullBackupPayload,
  restoreBackupPayloadForDatabase,
...
function sqlDumpEngine(engineLabel: string): 'postgres' | 'mysql' | 'mongodb' {
  const e = engineLabel.toLowerCase();
  if (e.includes('mongo')) return 'mongodb';
  if (e.includes('mysql') || e.includes('mariadb')) return 'mysql';
  return 'postgres';
}

export async function listBackups(userId: string, databaseId: string) {
...
export async function createBackup(userId: string, databaseId: string) {
...
export async function deleteBackup(userId: string, databaseId: string, backupId: string) {
  const { database } = await requireDatabaseProjectRole(databaseId, userId, UserRole.EDITOR, 'Наблюдатель не может удалять бэкапы');
  const backup = assertFound(
    await prisma.backup.findFirst({ where: { id: backupId, databaseId: database.id } }),
    'Бэкап не найден.',
  );
...
export async function restoreBackup(userId: string, databaseId: string, backupId: string) {
...
export async function getBackupSqlExport(userId: string, databaseId: string, backupId: string, includeData: boolean) {
  const { database } = await requireDatabaseProjectRole(databaseId, userId, UserRole.VIEWER);
  const backup = assertFound(await prisma.backup.findFirst({ where: { id: backupId, databaseId: database.id } }), 'Бэкап не найден.');
...
export async function getBackupDownload(userId: string, databaseId: string, backupId: string) {
  const { database } = await requireDatabaseProjectRole(databaseId, userId, UserRole.VIEWER);
  const backup = assertFound(await prisma.backup.findFirst({ where: { id: backupId, databaseId: database.id } }), 'Бэкап не найден.');
  return {
