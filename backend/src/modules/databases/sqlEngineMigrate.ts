...
export type SqlFamilyEngine = 'postgresql' | 'mysql';

function inferPayloadSourceEngine(payload: Record<string, unknown>): SqlFamilyEngine {
  const platform = String(payload.platformEngine ?? '').toLowerCase();
  if (platform.includes('mysql') || platform.includes('mariadb')) return 'mysql';
  if (platform.includes('postgres')) return 'postgresql';
  const tables = payload.tables as unknown[] | undefined;
  const firstCol = (tables?.[0] as { columns?: unknown[] } | undefined)?.columns?.[0] as Record<string, unknown> | undefined;
  if (firstCol && typeof firstCol.COLUMN_NAME === 'string') return 'mysql';
  return 'postgresql';
}

/** varchar(255) CHARACTER SET utf8mb4 COLLATE … → varchar(255) */
function normalizeMysqlColumnType(raw: string): string {
  return raw
...
function mapPgTypeToMysql(pgTypeRaw: string, dataType: string | undefined): string {
  ...
  if (pg.includes('[]')) return 'JSON';

  const uuidLike = pg === 'uuid';
  if (uuidLike) return 'CHAR(36)';
  ...
  if (pg === 'text' || pg.startsWith('text(')) return 'LONGTEXT';
  if (pg.startsWith('char(')) {
    const m = pg.match(/char\s*\(\s*(\d+)\s*\)/);
    return m ? `CHAR(${m[1]})` : 'CHAR(36)';
  }

  if (pg.includes('timestamp with time zone') || pg === 'timestamptz') return 'DATETIME(6)';
  if (pg.includes('timestamp') || pg === 'timestamp without time zone') return 'DATETIME';
  if (pg === 'date') return 'DATE';
  if (pg === 'time' || pg.startsWith('time ')) return 'TIME';

  if (pg === 'boolean' || pg === 'bool') return 'TINYINT(1)';
  if (pg === 'smallint' || pg === 'int2') return 'SMALLINT';
  if (pg === 'integer' || pg === 'int' || pg === 'int4') return 'INT';
  if (pg === 'bigint' || pg === 'int8') return 'BIGINT';
  if (pg === 'serial') return 'INT';
  if (pg === 'bigserial') return 'BIGINT';

  if (pg === 'json' || pg === 'jsonb') return 'JSON';
  ...
  if (pg === 'double precision' || pg === 'float8') return 'DOUBLE';
  if (pg === 'real' || pg === 'float4') return 'FLOAT';

  if (pg === 'bytea') return 'LONGBLOB';

  if (pg.startsWith('bit')) return 'BIT(1)';

  return 'LONGTEXT';
}

function mapMysqlTypeToPg(mysqlTypeRaw: string, extra: string): string {
  const raw = normalizeMysqlColumnType(mysqlTypeRaw);
  const low = raw.toLowerCase();
  const extraLow = String(extra ?? '').toLowerCase();

  if (low.startsWith('tinyint(1)') || low === 'bit(1)') return 'BOOLEAN';
  if (low.startsWith('tinyint')) return 'SMALLINT';
  if (low.startsWith('smallint')) return 'SMALLINT';
  if (low.startsWith('mediumint')) return 'INTEGER';
  if (low.startsWith('bigint')) return 'BIGINT';
  if (low.startsWith('int')) return 'INTEGER';

  if (low.startsWith('double') || low.startsWith('float')) return 'DOUBLE PRECISION';
  ...
  if (low.startsWith('json')) return 'JSONB';
  ...
  if (low.startsWith('date')) return 'DATE';
  if (low.startsWith('time')) return 'TIME WITHOUT TIME ZONE';
  ...
  if (low.startsWith('enum') || low.startsWith('set')) return 'TEXT';

  return 'TEXT';
}

function pgDataTypeFromPgType(pgType: string): string {
  const p = pgType.trim().toLowerCase();
  if (p.startsWith('character varying')) return 'character varying';
  if (p.startsWith('varchar')) return 'character varying';
  if (p.startsWith('timestamp with time zone') || p === 'timestamptz') return 'timestamp with time zone';
  if (p.startsWith('timestamp')) return 'timestamp without time zone';
  if (p.startsWith('time')) return 'time without time zone';
  const base = p.split(/\s+/)[0] ?? p;
  return base.replace(/[(),]/g, '') || 'text';
}

function normalizeCellForMysql(mysqlColumnType: string, value: unknown): unknown {
...
function normalizeCellForPostgres(pgType: string, value: unknown): unknown {
  if (value === null || value === undefined) return value === undefined ? null : value;
...
function convertRowsForMysql(columns: any[], primaryKeyColumns: string[], rows: any[]): any[] {
...
function convertRowsForPostgres(columns: any[], rows: any[]): any[] {
  const types = new Map<string, string>();
  for (const c of columns) {
...
function convertTablePostgresToMysql(table: any): any {
...
function convertTableMysqlToPostgres(table: any): any {
  const primaryKeyColumns = Array.isArray(table.primaryKeyColumns) ? [...table.primaryKeyColumns] : [];
  const cols = (table.columns ?? []).map((col: any) => {
    const name = String(col.COLUMN_NAME);
 ...
 */
export function convertSqlBackupPayloadForEngine(payload: Record<string, unknown>, target: SqlFamilyEngine): Record<string, unknown> {
  const source = inferPayloadSourceEngine(payload);
  if (source === target) {
    return { ...payload };
  }

  const tables = (payload.tables as any[]) ?? [];
  const convertedTables =
    target === 'mysql' ? tables.map(convertTablePostgresToMysql) : tables.map(convertTableMysqlToPostgres);
