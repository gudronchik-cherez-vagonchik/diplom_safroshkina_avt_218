...
export async function registerUser(input: { name: string; email: string; password: string }) {
  ...
  if (existing) throw new HttpError(409, 'Этот email уже зарегистрирован');
...
export async function updateUserProfile(userId: string, input: { name?: string; username?: string | null }) {
  const data: { name?: string; username?: string | null } = {};

  if (input.name !== undefined) {
    const n = input.name.trim();
    if (n.length < 2) throw new HttpError(400, 'Имя слишком короткое');
    data.name = n;
  }
  ...
  if (input.username !== undefined) {
    if (input.username === null || input.username.trim() === '') {
      ...
      if (taken) throw new HttpError(409, 'Этот username уже занят');
      data.username = u;
    }
  }
