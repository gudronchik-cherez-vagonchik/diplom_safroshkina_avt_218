export type DbEngineKind = 'postgres' | 'mysql' | 'mongodb';

function rawTail(msg: string, max = 280): string {
  const s = msg.trim();
...
export function explainDbRuntimeError(engine: DbEngineKind, error: unknown): string {
  ...
  if (engine === 'postgres' && pgCode) {
    ...
    if (map[pgCode]) return map[pgCode];
  }

  // MySQL / MariaDB (mysql2)
  const myNum = typeof (error as { errno?: number })?.errno === 'number' ? (error as { errno: number }).errno : undefined;
  const myCode = (error as { code?: string })?.code;
  if (engine === 'mysql') {
    ...
    if (myNum !== undefined && byNum[myNum]) return byNum[myNum];
    if (myCode === 'ER_ACCESS_DENIED_ERROR') return byNum[1045];
    if (myCode === 'ER_NO_SUCH_TABLE') return byNum[1146];
    if (myCode === 'ER_DUP_ENTRY') return byNum[1062];
  ...
  // MongoDB
  if (engine === 'mongodb') {
    const mCode = (error as { code?: number }).code;
    const byMongo: Record<number, string> = {
    ...
    if (mCode !== undefined && byMongo[mCode]) return byMongo[mCode];
    if (lower.includes('authentication failed')) return byMongo[18];
    if (lower.includes('not authorized')) return byMongo[13];
    if (lower.includes('duplicate key')) return byMongo[11000];
...
export function explainDbRuntimeErrorFromEngineLabel(engineLabel: string, error: unknown): string {
  const e = engineLabel.toLowerCase();
  let kind: DbEngineKind = 'postgres';
  if (e.includes('mongo')) kind = 'mongodb';
  else if (e.includes('mysql') || e.includes('maria')) kind = 'mysql';
