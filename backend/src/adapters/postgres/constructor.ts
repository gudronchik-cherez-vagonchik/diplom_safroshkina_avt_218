import { createPostgresPool, PostgresConnectionInput } from './adapter.js';

export type ConstructorColumn = {
  id?: string;
  name: string;
  type: string;
  primaryKey?: boolean;
...
export type ConstructorTable = {
  ...
  x?: number;
  y?: number;
  /** Только для UI конструктора, не участвует в DDL */
  color?: string;
  columns: ConstructorColumn[];
};

export type ConstructorRelation = {
  id?: string;
  fromTable: string;
  fromColumn: string;
  toTable: string;
...
export type ConstructorSchema = {
  tables: ConstructorTable[];
  relations?: ConstructorRelation[];
  viewport?: {
    zoom: number;
    panX: number;
    panY: number;
  };
  /** Только UI конструктора */
...
function isNextvalDefault(value: unknown) {
  return typeof value === 'string' && /nextval\(/i.test(value);
}

function mapDataType(column: any) {
  ...
  if (isNextvalDefault(defaultValue)) {
    if (udt === 'int2') return 'SMALLSERIAL';
    if (udt === 'int4') return 'SERIAL';
    if (udt === 'int8') return 'BIGSERIAL';
  }

  if (dataType === 'character varying') return maxLength ? `VARCHAR(${maxLength})` : 'VARCHAR(255)';
  if (dataType === 'character') return maxLength ? `CHAR(${maxLength})` : 'CHAR(1)';
  ...
  if (udt === 'numeric') return precision ? `NUMERIC(${precision}${scale !== null && scale >= 0 ? `, ${scale}` : ''})` : 'NUMERIC';
...
export async function exportConstructorSchema(connection: PostgresConnectionInput): Promise<ConstructorSchema> {
  ...
  try {
    ...
    const tables: ConstructorTable[] = [];
    const relations: ConstructorRelation[] = [];
    ...
    for (const [index, row] of tablesRes.rows.entries()) {
      ...
      tables.push({
        ...
        x: pos.x,
        y: pos.y,
        columns: columnsRes.rows.map((column: any) => ({
      ...
      for (const fk of fkRes.rows) {
        if (SYSTEM_TABLES.has(String(fk.to_table))) continue;
        relations.push({
...
async function getExistingColumns(client: any, tableName: string) {
  const res = await client.query(`
    SELECT column_name
    FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = $1
  `, [tableName]);
  return new Set(res.rows.map((row: any) => String(row.column_name)));
}

async function getExistingForeignKeys(client: any, tableNames: string[]) {
  ...
  `, [tableNames]);
  return res.rows as Array<{ table_name: string; constraint_name: string }>;
}
...
function isSerialType(type: string) {
  const value = normalizeType(type);
  return value === 'SERIAL' || value === 'BIGSERIAL' || value === 'SMALLSERIAL';
}

function serialBaseType(type: string) {
  const value = normalizeType(type);
  if (value === 'SMALLSERIAL') return 'SMALLINT';
  if (value === 'BIGSERIAL') return 'BIGINT';
  return 'INTEGER';
}

export async function applyConstructorSchema(connection: PostgresConnectionInput, schema: ConstructorSchema) {
  const pool = createPostgresPool(connection);
  ...
  try {
    ...
    for (const table of desiredTables) {
      ...
      if (!exists) {
        const columnsSql = table.columns.map((column) => {
          ...
          if ((column.nullable === false || column.primaryKey) && !isSerialType(normalizedType)) parts.push('NOT NULL');
          if (column.unique) parts.push('UNIQUE');
          if (column.defaultValue && !isSerialType(normalizedType)) parts.push(`DEFAULT ${column.defaultValue}`);
          return parts.join(' ');
        }).join(', ');
      ...
      for (const column of table.columns) {
        ...
        if (!existingColumns.has(column.name)) {
          ...
          if ((column.nullable === false || column.primaryKey) && !serialType) parts.push('NOT NULL');
          if (column.defaultValue && !serialType) parts.push(`DEFAULT ${column.defaultValue}`);
        ...
        if (column.unique) {
          await client.query(`ALTER TABLE ${quoteIdent(tableName)} ADD CONSTRAINT ${quoteIdent(uniqueName)} UNIQUE (${quoteIdent(column.name)})`).catch((error: any) => {
    ...
    for (const relation of schema.relations ?? []) {
      if (!desiredTableNames.has(relation.fromTable) || !desiredTableNames.has(relation.toTable)) continue;
      ...
      if (fromExists.rowCount === 0 || toExists.rowCount === 0) continue;
