...
export function buildConnectionString(input: MongoConnectionInput, authSource?: string) {
  const auth = input.user ? `${encode(input.user)}:${encode(input.password)}@` : '';
  const params = new URLSearchParams();
  params.set('authSource', authSource || input.database || 'admin');
  if (input.ssl) params.set('tls', 'true');
  return `mongodb://${auth}${input.host}:${input.port}/${input.database}?${params.toString()}`;
}

async function withClient<T>(input: MongoConnectionInput, fn: (client: MongoClient) => Promise<T>, authSource?: string) {
...
function collectFieldStats(doc: any, path = '', target = new Map<string, { count: number; types: Set<string> }>()) {
  if (!doc || typeof doc !== 'object' || Array.isArray(doc)) return target;
  for (const [key, value] of Object.entries(doc)) {
    const nextPath = path ? `${path}.${key}` : key;
    const entry = target.get(nextPath) ?? { count: 0, types: new Set<string>() };
    entry.count += 1;
...
function buildTree(stats: Map<string, { count: number; types: Set<string> }>, sampleSize: number): FieldNode[] {
  ...
  const walk = (obj: Record<string, any>): FieldNode[] => Object.entries(obj).map(([name, value]) => ({
    name,
    types: [...((value.__meta?.types as Set<string>) ?? new Set(['object']))],
    required: ((value.__meta?.count as number) ?? 0) === sampleSize,
    frequency: sampleSize === 0 ? 0 : Number((((value.__meta?.count as number) ?? 0) / sampleSize).toFixed(2)),
...
export async function testMongoConnection(connection: MongoConnectionInput) {
...
export async function provisionMongoDatabase(input: {
  ...
  return withClient(input.admin, async (client) => {
    ...
    const existingUsers = await targetDb.command({ usersInfo: 1 }).catch(() => ({ users: [] as any[] }));
    const exists = (existingUsers.users ?? []).some((user: any) => user.user === input.ownerUser);
    if (exists) {
...
export async function dropMongoManagedDatabase(admin: MongoConnectionInput, databaseName: string) {
  return withClient(admin, async (client) => {
    await client.db(databaseName).dropDatabase();
  }, admin.database || 'admin');
}

export async function listMongoCollections(connection: MongoConnectionInput) {
  return withClient(connection, async (client) => {
    const db = client.db(connection.database);
    const dbStats = await db.stats();
    const collections = await db.listCollections({}, { nameOnly: false }).toArray();

    const rows = [] as Array<{ name: string; columns: number; rows: number; size: string }>;
...
export async function getMongoCollectionData(connection: MongoConnectionInput, collection: string, limit: number, offset: number) {
  return withClient(connection, async (client) => {
    const db = client.db(connection.database);
...
export async function insertMongoDocument(connection: MongoConnectionInput, collection: string, data: Record<string, unknown>) {
  return withClient(connection, async (client) => {
    const db = client.db(connection.database);
...
export async function updateMongoDocument(connection: MongoConnectionInput, collection: string, pkValue: unknown, patch: Record<string, unknown>) {
  return withClient(connection, async (client) => {
    const db = client.db(connection.database);
    const payload = { ...patch } as any;
    delete payload._id;
    const _id = parseId(pkValue);
    await db.collection(collection).updateOne({ _id } as any, { $set: payload });
    const next = await db.collection(collection).findOne({ _id } as any);
    return next ? normalizeValue(next) : null;
  }, connection.database);
}

export async function deleteMongoDocument(connection: MongoConnectionInput, collection: string, pkValue: unknown) {
  return withClient(connection, async (client) => {
    const db = client.db(connection.database);
    const _id = parseId(pkValue);
    const result = await db.collection(collection).deleteOne({ _id } as any);
    return result.deletedCount ?? 0;
  }, connection.database);
}

export async function clearMongoCollection(connection: MongoConnectionInput, collection: string) {
  return withClient(connection, async (client) => {
    const db = client.db(connection.database);
...
export async function createMongoCollection(connection: MongoConnectionInput, name: string) {
  return withClient(connection, async (client) => {
    const db = client.db(connection.database);
...
export async function deleteMongoCollection(connection: MongoConnectionInput, name: string) {
  return withClient(connection, async (client) => {
    const db = client.db(connection.database);
...
export async function getMongoVisual(connection: MongoConnectionInput) {
  return withClient(connection, async (client) => {
    const db = client.db(connection.database);
    const collections = await db.listCollections({}, { nameOnly: false }).toArray();
    const visualCollections: any[] = [];
    ...
    for (const collection of collections) {
      ...
      const statsMap = docs.reduce((acc, doc) => collectFieldStats(doc, '', acc), new Map<string, { count: number; types: Set<string> }>());
      ...
      visualCollections.push({
        ...
        indexes: indexes.map((index: any) => ({ name: index.name, key: index.key, unique: !!index.unique, sparse: !!index.sparse, expireAfterSeconds: index.expireAfterSeconds ?? null })),
        fields,
      });
    }
...
export async function exportMongoBackupPayload(connection: MongoConnectionInput) {
  return withClient(connection, async (client) => {
    const db = client.db(connection.database);
    const collections = await db.listCollections({}, { nameOnly: true }).toArray();
    const payload: any = { collections: [] };
    for (const { name } of collections) {
      if (name.startsWith('system.')) continue;
      const docs = await db.collection(name).find({}).toArray();
      const indexes = await db.collection(name).indexes().catch(() => [] as any[]);
...
export async function restoreMongoBackupPayload(connection: MongoConnectionInput, payload: any) {
  return withClient(connection, async (client) => {
    const db = client.db(connection.database);
    const existing = await db.listCollections({}, { nameOnly: true }).toArray();
    for (const { name } of existing) {
      if (!name.startsWith('system.')) await db.collection(name).drop().catch(() => undefined);
    }
    for (const collection of payload.collections ?? []) {
      await db.createCollection(collection.name).catch(() => undefined);
      const coll = db.collection(collection.name);
      const docs = (collection.documents ?? []).map((doc: any) => {
        const next = { ...doc };
        if (typeof next._id === 'string' && /^[a-f0-9]{24}$/i.test(next._id)) next._id = new ObjectId(next._id);
        return next;
      });
      if (docs.length) {
