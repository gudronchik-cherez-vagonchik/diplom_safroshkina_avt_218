...
import type { ConnectionInput, DatabaseAdapterModule } from '../contracts/database-adapter.js';
import type { ConstructorSchema } from '../postgres/constructor.js';
 ...
 */
export function normalizeMysqlTcpHost(host: string): string {
  const h = host.trim().toLowerCase();
  if (h === 'localhost' || h === '::1') return '127.0.0.1';
  return host.trim();
}

function createConnection(input: ConnectionInput) {
...
function mapMySqlType(column: any) {
  ...
  if (column.extra?.toLowerCase().includes('auto_increment')) {
    if (dataType === 'bigint') return 'BIGINT AUTO_INCREMENT';
    return 'INTEGER AUTO_INCREMENT';
  }
  if (dataType === 'varchar') return `VARCHAR(${column.CHARACTER_MAXIMUM_LENGTH ?? column.character_maximum_length ?? 255})`;
  if (dataType === 'decimal') return `DECIMAL(${column.NUMERIC_PRECISION ?? column.numeric_precision ?? 10},${column.NUMERIC_SCALE ?? column.numeric_scale ?? 0})`;
  if (columnType) return columnType.toUpperCase();
...
async function testConnection(input: ConnectionInput) {
  ...
  try {
    const [rows] = await conn.query<any[]>('SELECT VERSION() AS version, DATABASE() AS database_name');
    return {
...
async function provisionManagedDatabase(input: { admin: ConnectionInput; databaseName: string; ownerUser: string; ownerPassword: string }) {
  ...
  try {
    ...
    for (const host of ["'%'", "'localhost'"] as const) {
...
export async function dropMysqlManagedDatabase(admin: ConnectionInput, databaseName: string, ownerUser: string) {
...
async function listSchema(input: ConnectionInput) {
  const conn = await createConnection(input);
  try {
...
async function getTableData(input: ConnectionInput, table: string, limit: number, offset: number) {
  const conn = await createConnection(input);
  try {
    const [colRows] = await conn.query<any[]>(`SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? ORDER BY ORDINAL_POSITION`, [table]);
    const [rows] = await conn.query<any[]>(`SELECT * FROM ${quoteIdent(table)} LIMIT ${Number(limit)} OFFSET ${Number(offset)}`);
    return {
...
async function runQuery(input: ConnectionInput, query: string, values: unknown[] = []) {
  const conn = await createConnection(input);
  try {
    const [rows, fields] = await conn.query<any[]>(query, values as any[]);
    return {
      columns: Array.isArray(fields) ? fields.map((field: any) => String(field.name)) : [],
      rows: Array.isArray(rows) ? rows as Record<string, unknown>[] : [],
      rowCount: Array.isArray(rows) ? rows.length : Number((rows as any)?.affectedRows ?? 0),
    };
  } finally {
    await conn.end();
  }
...
async function insertRow(input: ConnectionInput, table: string, data: Record<string, unknown>) {
  const conn = await createConnection(input);
  try {
    ...
    const [rows] = await conn.query<any[]>(`SELECT * FROM ${quoteIdent(table)} ORDER BY 1 DESC LIMIT 1`);
    return rows[0] ?? {};
  } finally {
    await conn.end();
  }
...
async function updateRow(input: ConnectionInput, table: string, pkColumn: string, pkValue: unknown, patch: Record<string, unknown>) {
  const conn = await createConnection(input);
  try {
    ...
    const [rows] = await conn.query<any[]>(`SELECT * FROM ${quoteIdent(table)} WHERE ${quoteIdent(pkColumn)} = ? LIMIT 1`, [pkValue]);
    return rows[0] ?? null;
  } finally {
    await conn.end();
  }
...
async function deleteRow(input: ConnectionInput, table: string, pkColumn: string, pkValue: unknown) {
  const conn = await createConnection(input);
  try {
    const [result] = await conn.query<any>(`DELETE FROM ${quoteIdent(table)} WHERE ${quoteIdent(pkColumn)} = ?`, [pkValue]);
...
function inferPrimaryKeyFromLogicalSchemaMysql(tableName: string, logicalSchema: unknown): string[] {
...
function buildMysqlCreateTableFromBackup(
  tableName: string,
  columns: any[],
  ...
  if (!pk.length) pk = inferPrimaryKeyFromLogicalSchemaMysql(tableName, logicalSchema);
  ...
  for (const col of columns) {
    ...
    if (col.COLUMN_DEFAULT != null && String(col.COLUMN_DEFAULT) !== '') {
      ...
      if (d.toUpperCase() !== 'NULL') def = ` DEFAULT ${col.COLUMN_DEFAULT}`;
    }
...
async function exportBackupPayload(input: ConnectionInput) {
  const conn = await createConnection(input);
  try {
    ...
    for (const table of schema) {
      ...
      const primaryKeyColumns = columns.filter((c: any) => String(c.COLUMN_KEY).toUpperCase() === 'PRI').map((c: any) => String(c.COLUMN_NAME));
      const [rows] = await conn.query<any[]>(`SELECT * FROM ${quoteIdent(table.name)}`);
...
async function restoreBackupPayload(input: ConnectionInput, payload: any) {
  const conn = await createConnection(input);
  try {
    ...
    const backupNames = new Set(backupTables.map((t: any) => String(t.name)));

    const [dbRows] = await conn.query<any[]>(
      `SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_TYPE = 'BASE TABLE'`,
    );
    for (const row of dbRows) {
      const name = String(row.TABLE_NAME);
      if (backupNames.has(name) || NEVER_DROP_MYSQL_TABLES.has(name)) continue;
    ...
    for (const table of backupTables) {
      for (const row of table.rows ?? []) {
        ...
        const placeholders = keys.map(() => '?').join(', ');
        await conn.query(
          `INSERT INTO ${quoteIdent(String(table.name))} (${keys.map(quoteIdent).join(', ')}) VALUES (${placeholders})`,
...
async function exportConstructorSchema(input: ConnectionInput): Promise<ConstructorSchema> {
  const conn = await createConnection(input);
  try {
    const [tableRows] = await conn.query<any[]>(`SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_TYPE = 'BASE TABLE' ORDER BY TABLE_NAME`);
    const tables = [] as ConstructorSchema['tables'];
    const relations = [] as NonNullable<ConstructorSchema['relations']>;
...
async function fetchMysqlColumnType(conn: any, tableName: string, columnName: string): Promise<{ columnType: string; extra: string } | null> {
  const [rows] = await conn.query<any[]>(
...
function normalizeMysqlColumnTypeDisplay(value: string) {
  return value.trim().replace(/\s+/g, ' ').toLowerCase();
}

/** Несколько выражений через `;` (DDL/DML). Отдельное соединение от интерактивных запросов. */
export async function runMysqlMigrationSql(input: ConnectionInput, sql: string) {
  const conn = await mysql.createConnection({
    host: normalizeMysqlTcpHost(input.host),
    port: input.port,
    database: input.database,
...
export async function truncateMysqlTable(input: ConnectionInput, table: string) {
  await runMysqlMigrationSql(input, `TRUNCATE TABLE ${quoteIdent(table)}`);
}

async function applyConstructorSchema(input: ConnectionInput, schema: ConstructorSchema) {
  const conn = await createConnection(input);
  try {
    await conn.beginTransaction();
    await conn.query('SET FOREIGN_KEY_CHECKS = 0');

    const [existingRows] = await conn.query<any[]>(`SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_TYPE = 'BASE TABLE'`);
    ...
    for (const table of schema.tables) {
      if (!existing.has(table.name)) {
        const cols = table.columns.map((column) => {
          ...
          if ((column.nullable === false || column.primaryKey) && !normalizeType(column.type).includes('AUTO_INCREMENT')) parts.push('NOT NULL');
          if (column.defaultValue && !normalizeType(column.type).includes('AUTO_INCREMENT')) parts.push(`DEFAULT ${column.defaultValue}`);
          return parts.join(' ');
        }).join(', ');
      ...
      const [columnRows] = await conn.query<any[]>(`SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ?`, [table.name]);
      ...
      for (const column of table.columns) {
        ...
        if (!existingColumns.has(column.name)) {
          ...
          if ((column.nullable === false || column.primaryKey) && !normalizeType(column.type).includes('AUTO_INCREMENT')) clauses.push('NOT NULL');
          if (column.defaultValue && !normalizeType(column.type).includes('AUTO_INCREMENT')) clauses.push(`DEFAULT ${column.defaultValue}`);
    ...
    for (const relation of schema.relations ?? []) {
      if (!desired.has(relation.fromTable) || !desired.has(relation.toTable)) continue;

      const [fromCol] = await conn.query<any[]>(
        `SELECT 1 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ? LIMIT 1`,
        [relation.fromTable, relation.fromColumn],
      );
      const [toCol] = await conn.query<any[]>(
        `SELECT 1 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ? LIMIT 1`,
        [relation.toTable, relation.toColumn],
      );
      if (!fromCol?.length || !toCol?.length) continue;
      ...
      if (refMeta && fkMeta) {
        ...
        if (refT !== fkT) {
          ...
          if (!fromColSchema) {
            throw new Error(
              `Несовместимые типы FK: ${relation.fromTable}.${relation.fromColumn} (${fkMeta.columnType}) → ${relation.toTable}.${relation.toColumn} (${refMeta.columnType}). Укажите столбец в схеме конструктора.`,
            );
          }
          if (fkMeta.extra.toLowerCase().includes('auto_increment')) {
            throw new Error(
              `Столбец ${relation.fromTable}.${relation.fromColumn} с AUTO_INCREMENT нельзя автоматически привести к типу «${refMeta.columnType}» для внешнего ключа. Задайте совместимый тип вручную.`,
            );
          }
          const nullClause = fromColSchema.nullable === false ? 'NOT NULL' : 'NULL';
          await conn.query(
            `ALTER TABLE ${quoteIdent(relation.fromTable)} MODIFY COLUMN ${quoteIdent(relation.fromColumn)} ${refMeta.columnType} ${nullClause}`,
          );
        }
      }
...
export type MysqlConnectionInput = ConnectionInput;

export {
  testConnection as testMysqlConnection,
  provisionManagedDatabase as provisionMysqlDatabase,
  listSchema as listMysqlSchema,
  getTableData as getMysqlTableData,
  runQuery as runMysqlSql,
  insertRow as insertMysqlRow,
  updateRow as updateMysqlRow,
  deleteRow as deleteMysqlRow,
  exportBackupPayload as exportMysqlBackupPayload,
  restoreBackupPayload as restoreMysqlBackupPayload,
  exportConstructorSchema as exportMysqlConstructorSchema,
  applyConstructorSchema as applyMysqlConstructorSchema,
};

export const mysqlAdapter: DatabaseAdapterModule = {
  engine: 'mysql',
  title: 'MySQL',
  defaultPort: 3306,
  queryLanguage: 'sql',
